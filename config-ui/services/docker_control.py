...
# Generated from FOUNDRY_ASSETS_PATH (see sync_assets_mount) rather than a
# line in docker-compose.yml's own `volumes:` list. This process drives
# `docker compose` from inside its own container, over the mounted Docker
# socket (Docker-outside-of-Docker, see config-ui/Dockerfile) - so a
# *relative* bind-mount source in the base compose file would resolve
# against this container's own filesystem view (e.g. /workspace) instead of
# the real host's, and there's no absolute path that's a safe universal
# default for "nothing mounted." An override file that's simply absent when
# unset sidesteps that: nothing is asserted about the mount unless the user
# actually sets an absolute host path.
ASSETS_OVERRIDE_FILE = "/workspace/docker-compose.assets.yml"
ASSETS_MOUNT_TARGET = "/data/Data/Mount"
...
SETTINGS_STEPS = [
    ...
    (
        ...
        "Mount a folder from this machine straight into Foundry, so its contents show up "
        "under Data/Mount in Foundry's own file picker - no need to upload anything by hand.",
        [
            ("FOUNDRY_ASSETS_PATH", "Assets Folder Path", "text",
             "Optional. An absolute path on this machine, e.g. C:\\Users\\you\\FoundryAssets "
             "(Windows) or /home/you/foundry-assets (Linux/Mac). Shows up in Foundry's file "
             "picker as Data/Mount - named that way so it's obviously the folder from your "
             "machine, not something Foundry itself created. Leave blank to skip - nothing "
             "is mounted by default. Changing this restarts Foundry."),
        ],
    ),
    (
...
# Fields that only mean anything when typed in from the same machine running
# Docker - a path like C:\Users\you\FoundryAssets is meaningless if you're
# looking at the dashboard from another device on the LAN or over the
# Cloudflare Tunnel, since it's always resolved against the Docker host's
# filesystem, never the browsing device's. Hidden from the wizard (see
# settings_steps below) unless the request is coming from localhost itself.
LOCALHOST_ONLY_FIELDS = {"FOUNDRY_ASSETS_PATH"}


def settings_steps(is_localhost: bool) -> list:
    """SETTINGS_STEPS, with LOCALHOST_ONLY_FIELDS (and any step left with no
    fields once those are dropped) filtered out for non-localhost requests."""
    if is_localhost:
        return SETTINGS_STEPS
    filtered = []
    for title, subtitle, fields in SETTINGS_STEPS:
        remaining = [f for f in fields if f[0] not in LOCALHOST_ONLY_FIELDS]
        if remaining:
            filtered.append((title, subtitle, remaining))
    return filtered
...
def sync_assets_mount(host_path: str) -> None:
    """Writes or removes docker-compose.assets.yml so the foundry service's
    bind mount at ASSETS_MOUNT_TARGET matches FOUNDRY_ASSETS_PATH. Called on
    every Foundry Settings save (see save() in app.py) and once at startup,
    so it self-heals if the override file and .env ever drift apart (e.g.
    someone edits .env by hand, or the file is deleted)."""
    host_path = (host_path or "").strip()
    if not host_path:
        try:
            os.remove(ASSETS_OVERRIDE_FILE)
        except FileNotFoundError:
            pass
        return
    # Double-quoted YAML scalar: backslashes (Windows paths) and quotes need
    # escaping. Compose's volume-spec parser recognizes a Windows drive
    # letter (e.g. "C:\...") and doesn't mistake its colon for the
    # source:target separator, regardless of what OS `docker compose` itself
    # runs on (confirmed with `docker compose config` against this exact
    # short-syntax form).
    escaped = host_path.replace("\\", "\\\\").replace('"', '\\"')
    content = (
        "# Generated by config-ui from FOUNDRY_ASSETS_PATH in .env - do not edit by hand,\n"
        "# your changes will be overwritten on the next Foundry Settings save.\n"
        "services:\n"
        "  foundry:\n"
        "    volumes:\n"
        f'      - "{escaped}:{ASSETS_MOUNT_TARGET}"\n'
    )
    with open(ASSETS_OVERRIDE_FILE, "w", encoding="utf-8") as f:
        f.write(content)


def _compose_file_args() -> list[str]:
    args = ["-f", COMPOSE_FILE]
    if os.path.exists(ASSETS_OVERRIDE_FILE):
        args += ["-f", ASSETS_OVERRIDE_FILE]
    return args
